...
/**
 * Logger interface for UserSyncService
 */
export interface UserSyncServiceLogger {
 ...
 */
const defaultLogger: UserSyncServiceLogger = {
  info: (data, msg) => console.log(`[UserSyncService] ${msg}`, data),
  warn: (msg) => console.warn(`[UserSyncService] ${msg}`),
  error: (data, msg) => console.error(`[UserSyncService] ${msg}`, data),
  debug: (msg) => console.debug(`[UserSyncService] ${msg}`),
};

/**
 * Database pool interface
 * Compatible with node-postgres Pool
 */
export interface DatabasePool {
  query: (query: string, values?: unknown[]) => Promise<{ rows: User[]; rowCount: number | null }>;
}

/**
 * User sync service configuration
 */
export interface UserSyncServiceConfig {
  /** Database pool instance */
  pool: DatabasePool;
  /** Logger instance (optional) */
  logger?: UserSyncServiceLogger;
}

/**
 * Service for synchronizing Cognito users to the database
 */
export class UserSyncService {
  private readonly pool: DatabasePool;
  private readonly logger: UserSyncServiceLogger;

  /**
   * Create a new UserSyncService
   * @param config - Service configuration with database pool
   */
  constructor(config: UserSyncServiceConfig) {
  ...
  async syncUser(cognitoPayload: CognitoJwtPayload): Promise<User> {
    try {
      ...
      if (existingUser) {
        this.logger.debug(`User already exists in database: ${cognitoSub}`);
        return existingUser;
      }

      // User doesn't exist - create new record
      this.logger.info({}, `Creating new user record for ${cognitoSub}`);
      ...
      this.logger.error({ err: error }, 'Failed to sync user');
  ...
  async findUserByCognitoSub(cognitoSub: string): Promise<User | null> {
    try {
      ...
      const result = await this.pool.query(query, [cognitoSub]);
      ...
      this.logger.error({ err: error }, 'Failed to find user by cognito_sub');
      throw error;
    }
  }
  ...
  async updateUser(
    ...
    try {
      ...
      this.logger.info({ updates }, `User updated: ${cognitoSub}`);

      return result.rows[0] as User;
    } catch (error) {
      this.logger.error({ err: error }, 'Failed to update user');
      throw error;
    }
  }
  ...
  async deleteUser(cognitoSub: string): Promise<boolean> {
    try {
      ...
      if (result.rowCount === 0) {
        this.logger.warn(`User not found for deletion: ${cognitoSub}`);
        return false;
      }

      this.logger.info({}, `User deleted (soft): ${cognitoSub}`);
      return true;
    } catch (error) {
      this.logger.error({ err: error }, 'Failed to delete user');
      throw error;
    }
  }
}
